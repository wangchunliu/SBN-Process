...
def split_list_sbn(f):
    ...
    for sbn in all_sbns:
        if len(sbn) > 2:
            if re.search('p[0-9][0-1]/d\d{4}', sbn[1]):
                m = re.search('p[0-9][0-1]/d\d{4}', sbn[1])
                print("dev_dir:{}".format(m.group(0)))
                dev_data_dir.append(m.group(0))
            elif re.search('p[0-9][2-4]/d\d{4}', sbn[1]):
                m = re.search('p[0-9][2-4]/d\d{4}', sbn[1])
                print("test_dir:{}".format(m.group(0)))
                test_data_dir.append(m.group(0))
            else:
                m = re.search('p\d{2}/d\d{4}', sbn[1])
                print("train_dir:{}".format(m.group(0)))
                train_data_dir.append(m.group(0))
...
def get_dir_sen(inputpath, sen_dir, outfile):
    with open(outfile,'w') as out_f:
        for line in sen_dir:
            raw_path = os.path.join(inputpath, "{}/it.raw".format(line.strip()))
...
def get_dir_sbn(inputpath, sbn_dir, outfile):
    with open(outfile,'w') as out_f:
        for line in sbn_dir:
            sbn_path = os.path.join(inputpath, "{}/it.drs.sbn".format(line.strip()))
...
if __name__ == "__main__":
    ...
    train_data_dir, dev_data_dir, test_data_dir = split_list_sbn(args.input)
